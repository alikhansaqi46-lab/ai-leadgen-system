...
export default function WhatsAppBrainPage() {
  return <BrainConfigPage channel={CHANNEL} label={CHANNEL_LABEL} icon={CHANNEL_ICON} />;
}

function BrainConfigPage({ channel, label, icon }: { channel: ChannelType; label: string; icon: string }) {
  ...
  async function loadConfig() {
    try {
      ...
      setMessage({ type: 'error', text: `Failed to load ${label} Brain configuration.` });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave() {
    try {
      ...
      if (res.success) {
        setConfig({ ...DEFAULT_CONFIG, ...res.config });
        setMessage({ type: 'success', text: `${label} Brain configuration saved successfully.` });
      } else {
  ...
  if (loading) {
    return (
      <div className="lf-page">
        <PageHeader title={`${label} Brain`} subtitle={`Loading ${label} AI Brain configuration…`} />
  ...
  return (
    <div className="lf-page">
      <PageHeader
        title={`${icon} ${label} Brain`}
        subtitle={`Configure the ${label} AI Brain — independent from other channels`}
        actions={
          <button
      ...
      <div className="lf-note" style={{ marginBottom: 20 }}>
        This {label} Brain is completely independent. It has its own knowledge, prompts, tone, 
      ...
      <div className="lf-card" style={{ marginBottom: 24 }}>
        ...
        <label className="lf-checkbox-row" style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
          ...
          />
          <span>Enable AI for {label} channel</span>
        </label>
        <p style={{ fontSize: 12, color: 'var(--lf-muted)', margin: '8px 0 0 0' }}>
          When disabled, {label} messages will not be automatically replied to by the AI.
        </p>
      </div>
      ...
      <div className="lf-card" style={{ marginBottom: 24 }}>
        ...
        <div className="lf-field">
          ...
          <textarea
            ...
            placeholder="Override the default system prompt for this channel's AI brain. Leave empty to use the default."
          />
          <p style={{ fontSize: 12, color: 'var(--lf-muted)', margin: '4px 0 0 0' }}>
            This prompt is unique to the {label} channel. Other channels will not use it.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 8 }}>
        <button
          ...
          style={{ minWidth: 180 }}
        >
          {saving ? 'Saving…' : `Save ${label} Brain Configuration`}
        </button>
      </div>
    </div>
  );
